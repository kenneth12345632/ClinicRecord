...
@php
    $editRole = strtolower(trim((string) (auth()->user()->role ?? '')));
    $canEditStock = $editRole === 'bhw';
...
<div class="max-w-5xl mx-auto py-6 px-4 pb-14">
    <div class="mb-6">
        <a href="{{ route('medicines.index') }}" class="text-green-600 hover:text-green-800 flex items-center gap-2 mb-3 text-sm font-medium transition">
            ...
            <div class="space-y-5">
                <div>
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <label class="inline-flex items-center gap-1.5 text-[11px] font-semibold text-green-600">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-[1fr_120px] gap-3">
                        <input type="number" name="dosage_value" id="dosage_value" placeholder="e.g. 500" min="0.01" step="0.01" required value="{{ $dosageVal }}"
                            class="w-full px-5 py-3.5 rounded-xl border border-gray-200 focus:border-green-400 focus:ring-4 focus:ring-green-50 outline-none text-base font-medium transition">
                        <select name="dosage_unit" id="dosage_unit" required class="w-full px-4 py-3.5 rounded-xl border border-gray-200 focus:border-green-400 focus:ring-4 focus:ring-green-50 outline-none text-base font-medium transition bg-white">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="{{ $canEditStock ? '' : 'opacity-60' }} relative">
                        @unless($canEditStock)
                            <div class="absolute inset-0 z-10 cursor-not-allowed" onclick="alert('Only BHW can edit Date Received.')"></div>
                        @endunless
                        ...
                        <div class="relative w-full">
                            <input type="text" name="arrival_date" id="arrival_date" required autocomplete="off"
                                ...
                                {{ $canEditStock ? '' : 'readonly' }}
                                class="w-full px-5 py-3.5 pr-12 rounded-xl border border-gray-200 {{ $canEditStock ? 'focus:border-green-400 focus:ring-4 focus:ring-green-50' : 'bg-gray-50 cursor-not-allowed' }} outline-none text-base font-semibold text-gray-900 transition">
                    ...
                    <div class="{{ $canEditStock ? '' : 'opacity-60' }} relative">
                        @unless($canEditStock)
                            <div class="absolute inset-0 z-10 cursor-not-allowed" onclick="alert('Only BHW can edit Expiration Date.')"></div>
                        @endunless
                    ...
                    <div class="{{ $canEditStock ? '' : 'opacity-60' }} relative">
                        @unless($canEditStock)
                            <div class="absolute inset-0 z-10 cursor-not-allowed" onclick="alert('Only BHW can edit Quantity.')"></div>
                        @endunless
                        ...
                        <input type="number" name="stock" min="0" required value="{{ old('stock', $medicine->stock) }}"
                            {{ $canEditStock ? '' : 'readonly' }}
                            class="w-full px-5 py-3.5 rounded-xl border border-gray-200 {{ $canEditStock ? 'focus:border-green-400 focus:ring-4 focus:ring-green-50' : 'bg-gray-50 cursor-not-allowed' }} outline-none text-base font-medium transition">
                    </div>
                </div>
            </div>

            <div class="mt-8 pt-6 border-t border-gray-50 flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                <button type="submit" class="flex-1 py-3.5 bg-green-600 text-white text-sm font-black rounded-xl hover:bg-green-700 shadow-md transition">
                    Save changes
                </button>
