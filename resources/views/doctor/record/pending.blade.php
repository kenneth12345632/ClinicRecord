...
<div class="max-w-7xl mx-auto pb-20 px-4 sm:px-6 lg:px-8 mt-8">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
            ...
            <p class="text-slate-500 text-sm mt-1">Priority-based list of patient records waiting for consultation. Critical vitals highlighted automatically.</p>
        </div>
        <div class="w-full md:w-auto flex gap-2">
            <input type="text" id="searchInput" placeholder="Search pending patient records..." class="pl-4 pr-4 py-2.5 w-full md:w-80 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:ring-blue-500 outline-none shadow-sm">
        ...
        <table class="min-w-full">
            <thead class="bg-slate-100/80 border-b border-slate-200">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-[10px] font-black text-slate-500 uppercase">Patient Record Identity</th>
            ...
            <tbody id="recordsTableBody" class="divide-y divide-slate-200">
                ...
                @empty
                    <tr>
                        <td colspan="9" class="px-6 py-16 text-center text-gray-400 italic">No pending patient records in queue.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
function renderPendingPagination(filteredRows) {
    if (filteredRows.length === 0) {
        renderPaginationTable({
            ...
            rows: [],
            emptyRowHtml: '<tr><td colspan="9" class="px-6 py-16 text-center text-gray-400 italic">No pending patient records in queue.</td></tr>'
        });
        return;
    }
